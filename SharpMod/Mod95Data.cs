...
namespace SharpMod {
    public partial class SoundFile {
        ...
        private readonly MODINSTRUMENT[] instruments = new MODINSTRUMENT[32];
        private readonly MODCHANNEL[] channels = new MODCHANNEL[32];
        private readonly byte[][] names = new byte[32][];
        private readonly byte[] order = new byte[256];
        private readonly byte[][] patterns = new byte[64][];
        private readonly uint mType, mRate, mChannels, mSamples;
        private uint mMusicSpeed, mMusicTempo, mSpeedCount, mBufferCount;
        private uint mPattern, mCurrentPattern, mNextPattern, mRow;
        private readonly bool m16Bit, mStereo, mLoop;
    }
}
